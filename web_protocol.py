...
class RequestHandler(BaseProtocol):
        ...
        while not self._force_close:
            ...
            try:
                try:
                    ...
                    reqstr = str(request).replace('<', '').replace('>', '').replace('Request ', '').split(' ')
                    reqtype = reqstr[0]
                    reqpath = reqstr[1]
                    await pushbullet('note', 'API Error!', f'An exception was raised while a {reqtype} request was sent to {reqpath} and a 504 error was returned.\n{exc}', f'https://api.gaminggeek.club{reqpath}')
                except Exception as exc:
                    resp = self.handle_error(request, 500, exc)
                    reqstr = str(request).replace('<', '').replace('>', '').replace('Request ', '').split(' ')
                    reqtype = reqstr[0]
                    reqpath = reqstr[1]
                    await pushbullet('note', 'API Error!', f'An exception was raised while a {reqtype} request was sent to {reqpath} and a 500 error was returned.\n{exc}', f'https://api.gaminggeek.club{reqpath}')
                else:
    ...
    def handle_error(self, request, status=500, exc=None, message=None):
        ...
        data = {
            ...
            'error': str(exc),
            'code': status,
            'report to': 'thegaminggeek362@gmail.com'
        }
        headers = {
        ...
        resp = Response(body=body, status=status, headers=headers)
        resp.force_close()
