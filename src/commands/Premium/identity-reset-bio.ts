...
export default class IdentityResetBio extends Command {
  constructor() {
    super("identity-reset-bio", {
      description: (language: Language) =>
        language.get("IDENTITY_RESET_BIO_COMMAND_DESCRIPTION"),
  ...
  async run(command: ApplicationCommandMessage) {
    const updated = await this.client.req
      ...
      .catch(() => {});
    if (updated) {
      await command.guild.settings.delete("identity.bio", command.author);
