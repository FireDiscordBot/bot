...
class Music:
	...
	async def track_callback(self, player):
		...
		if not queue_data[1]:  # no more songs left
			...
			em = discord.Embed(title=f"▶ Now Playing", timestamp=datetime.datetime.now(), url=track.url)
	...
	async def play(self, ctx, *, query: str):
		...
		if self.queue.get(ctx.guild):
			...
			if is_playlist:
				...
				em = discord.Embed(title=f"▶ Playing a playlist with {len(tracks)} tracks in {ctx.guild}", color=ctx.message.author.color, timestamp=datetime.datetime.now(), url=track.url)
				em.add_field(name="Track", value=track.title)
				em.add_field(name="Author", value=track.author)
				em.add_field(name="Length", value=str(timedelta(milliseconds=track.length)))
				em.add_field(name="Volume", value=f"{player.volume}%")
				...
				em = discord.Embed(title=f"▶ Now Playing in {ctx.guild}", color=ctx.message.author.color, timestamp=datetime.datetime.now(), url=track.url)
				em.add_field(name="Track", value=track.title)
				em.add_field(name="Author", value=track.author)
				em.add_field(name="Length", value=str(timedelta(milliseconds=track.length)))
				em.add_field(name="Volume", value=f"{player.volume}%")
